...
public class ExecutorsTest {
     ...
     */
    public static void testException1() throws InterruptedException {
        ThreadPoolExecutor executor = buildThreadPoolTaskExecutor();

        executor.execute(() -> sayException("execute"));       // 堆栈异常直接输出

        Thread.sleep(100);

        executor.submit(() -> sayException("submit"));
    }
     ...
     * @throws InterruptedException
     */
    public static void testException2() throws InterruptedException {
        ThreadPoolExecutor executor = buildThreadPoolTaskExecutor();
        ...
        Future<?> submit = executor.submit(() -> sayException("submit"));     // 堆栈异常没有输出
     ...
     * @throws InterruptedException
     */
    public static void testThreadHandle() throws InterruptedException {
        ThreadPoolExecutor executor = buildThreadPoolTaskExecutor();

        executor.execute(() -> {
            sayException2("execute");
        });
        Thread.sleep(100);
        ...
        executor.execute(() -> {
            sayException2("execute-exception");
        });
        Thread.sleep(100);
...
//        testException2();
     ...
     * @param name
     */
    private static void sayException(String name) {
     ...
     * @param name
     */
    private static void sayException2(String name) {
        String printStr = "[thread-name:" + Thread.currentThread().getName() + ", 执行方式：" + name + "]";
        System.out.println(printStr);
